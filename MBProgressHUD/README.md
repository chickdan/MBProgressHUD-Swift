...
# :warning: This project is a WIP :construction:
###This repository is a re-write of [MBProgressHUD](https://github.com/jdg/MBProgressHUD) in Swift and contains _most_ open pull requests from the original repository.

`MBProgressHUD-Swift` is an iOS drop-in class that displays a translucent HUD with an indicator and/or labels while work is being done in a background thread. The HUD is meant as a replacement for the undocumented, private `UIKit` `UIProgressHUD` with some additional features.
...
`MBProgressHUD` works on iOS 11.0+. It depends on the following Apple frameworks, which should already be included with most Xcode templates:

* Foundation.framework
* UIKit.framework
* CoreGraphics.framework

You will need the latest developer tools in order to build `MBProgressHUD-Swift`. Old Xcode versions might work, but compatibility will not be explicitly maintained.
...
1. Add MBProgressHUD to your Cartfile. e.g., `github "chickdan/MBProgressHUD-Swift" ~> 1.2.0`
